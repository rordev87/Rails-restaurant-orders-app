class HomeController < ApplicationController
  ...
  def getorderbyfriends
    ...
    @orders_of_friends = OrderUserJoin.where(user_id: friends_ids).order(:created_at => :desc).limit(5).map{|m| Order.find(m.order_id)} + Order.where( user_id: friends_ids).order(:created_at => :desc).limit(5).map{ |o| {:admin => User.find(o.user_id) , :order => o } }
